...
def test_get_all_ingredients(
    ...
    assert response.data[0] == IngredientSerializer(created_object).data
...
def test_create_ingredient(
    ...
    response = client.post(
        reverse(list_viewname), IngredientSerializer(resource_data).data
    )
...
def test_get_ingredient(
    ...
    assert response.status_code == expected_status
    assert response.data == IngredientSerializer(created_object).data
